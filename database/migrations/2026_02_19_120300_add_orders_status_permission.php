...
return new class extends Migration {
    public function up(): void
    {
        DB::table('permissions')->updateOrInsert(
            ['name' => 'orders.status', 'guard_name' => 'web'],
            ['name' => 'orders.status', 'guard_name' => 'web']
        );
        $permId = DB::table('permissions')->where('name', 'orders.status')->value('id');
        $roles = DB::table('roles')->whereIn('name', ['Admin','Manager','Kitchen Staff'])->pluck('id','name');
